const metricMap={coins:'w.coins_earned',xp:'x.damper_xp',wins:'s.games_won',games:'s.games_played',rpg:'x.rpg_xp',collector:'(COALESCE(c.cards,0)+COALESCE(p.pets,0)+COALESCE(n.numbered,0))',achievements:'COALESCE(a.achievements,0)',wager:'COALESCE(wg.staked,0)'};

export async function leaderboard(env,metric='coins',limit=10){
  const key=metricMap[metric]?'':null;if(key===null)return [];
