...
async function handle(env,update){const msg=update.message,q=update.callback_query;
  ...
  if(!q)return false;const d=q.data||'';if(!d.startsWith('rpg_')&&!d.startsWith('shop_')&&!d.startsWith('vault_')&&!d.startsWith('leaderboard:')&&d!=='leaderboard_main'&&!d.startsWith('rpg_buy:')&&d!=='menu_main'&&d!=='menu_games')return false;
