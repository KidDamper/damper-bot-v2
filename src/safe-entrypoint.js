...
export default {async fetch(request,env,ctx){if(request.method==='POST'&&new URL(request.url).pathname==='/telegram/webhook'){const clone=request.clone();try{const update=await clone.json();await ensureOwner(env,update.message);if(update.message?.text?.trim().split(/\\s+/)[0].toLowerCase()==='/walletdebug'){
  const chat=update.message.chat.id,id=String(update.message.from.id);
  ...
  const results=await db.prepare('SELECT user_id,COUNT(*) games,COALESCE(SUM(stake),0) staked,COALESCE(SUM(payout),0) payouts FROM game_results WHERE user_id IN (?,?) GROUP BY user_id ORDER BY user_id').bind(...ids).all();
  const stats=await db.prepare('SELECT user_id,games_played,games_won,games_lost,total_staked,total_won,total_lost FROM stats WHERE user_id IN (?,?) ORDER BY user_id').bind(...ids).all();
  const sessions=await db.prepare('SELECT player_id,COUNT(*) count,MAX(created_at) latest FROM game_sessions WHERE player_id IN (?,?) GROUP BY player_id ORDER BY player_id').bind(...ids).all();
  const fmtUsers=rows=>(rows||[]).map(x=>'#'+x.id+' tg='+x.telegram_id+' damper='+x.damper_id+' role='+x.role).join('\\n')||'None';
  const fmtRows=(rows,fn)=>(rows||[]).map(fn).join('\\n')||'None';
  const lineWallets=fmtRows(wallets.results,x=>'#'+x.user_id+': '+x.balance);
