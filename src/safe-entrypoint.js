...
const APP_VERSION='owner-001-wallet-debug-primary-20260925-r3';
const OWNER_TELEGRAM_ID='7852229418';
const OWNER_DAMPER_ID='001';
...
async function ensureOwner(env,msg){
  const id=String(msg?.from?.id||'');
  if(id!==OWNER_TELEGRAM_ID)return;
  const db=env.DB;
  let u=await db.prepare('SELECT id,damper_id,role FROM users WHERE telegram_id=?').bind(id).first();
  if(!u){
    const username=msg.from?.username||null;
    const existing=await db.prepare('SELECT id FROM users WHERE damper_id=?').bind(OWNER_DAMPER_ID).first();
    if(existing)throw Error('OWNER_DAMPER_ID_IN_USE');
    await db.prepare('INSERT INTO users(telegram_id,username,damper_id,role,created_at) VALUES(?,?,?,?,?)').bind(id,username,OWNER_DAMPER_ID,'OWNER',now()).run();
    u=await db.prepare('SELECT id FROM users WHERE telegram_id=?').bind(id).first();
    if(!u)throw Error('OWNER_CREATE_FAILED');
    await db.prepare('INSERT OR IGNORE INTO wallets(user_id,balance) VALUES(?,500)').bind(u.id).run();
    await db.prepare('INSERT OR IGNORE INTO xp(user_id,damper_xp,level,rpg_xp,rpg_level) VALUES(?,0,1,0,1)').bind(u.id).run();
    return;
  }
  if(String(u.role||'').toUpperCase()!=='OWNER'||String(u.damper_id)!==OWNER_DAMPER_ID){
    const conflict=await db.prepare('SELECT id FROM users WHERE damper_id=? AND id<>?').bind(OWNER_DAMPER_ID,u.id).first();
    if(conflict)throw Error('OWNER_DAMPER_ID_IN_USE');
    await db.prepare('UPDATE users SET role=?,damper_id=?,username=COALESCE(?,username) WHERE id=?').bind('OWNER',OWNER_DAMPER_ID,msg.from?.username||null,u.id).run();
  }
}
async function allowed(env,id){if(String(id)===OWNER_TELEGRAM_ID)return true;if(env.OWNER_TELEGRAM_ID&&String(env.OWNER_TELEGRAM_ID)===String(id))return true;return member(env,MEME,id)&&member(env,UPDATES,id);}
...
export default {async fetch(request,env,ctx){if(request.method==='POST'&&new URL(request.url).pathname==='/telegram/webhook'){const clone=request.clone();try{const update=await clone.json();await ensureOwner(env,update.message);if(update.message?.text?.trim().split(/\s+/)[0].toLowerCase()==='/walletdebug'){const chat=update.message.chat.id,id=String(update.message.from.id);if(id!==OWNER_TELEGRAM_ID&&!(env.OWNER_TELEGRAM_ID&&String(env.OWNER_TELEGRAM_ID)===id)){await send(env,chat,'⛔ Creator access only.');return new Response('OK');}const db=env.DB.withSession('first-primary');const u=await db.prepare('SELECT id,telegram_id,username,damper_id,role FROM users WHERE telegram_id=? ORDER BY id ASC LIMIT 1').bind(id).first();if(!u){await send(env,chat,'🔧 WALLET DEBUG\\n\\nNo users row found for Telegram ID: '+id);return new Response('OK');}if(String(u.role||'').toUpperCase()!=='OWNER'&&id!==OWNER_TELEGRAM_ID&&!(env.OWNER_TELEGRAM_ID&&String(env.OWNER_TELEGRAM_ID)===id)){await send(env,chat,'⛔ Creator access only.');return new Response('OK');}const w=await db.prepare('SELECT user_id,balance FROM wallets WHERE user_id=?').bind(u.id).first();const g=await db.prepare('SELECT COUNT(*) count,COALESCE(SUM(stake),0) staked,COALESCE(SUM(payout),0) payout FROM game_results WHERE user_id=?').bind(u.id).first();const s=await db.prepare('SELECT games_played,games_won,games_lost,total_staked,total_won,total_lost FROM stats WHERE user_id=?').bind(u.id).first();const sessions=await db.prepare('SELECT id,game_type,stake,result,reward_applied,created_at FROM game_sessions WHERE player_id=? ORDER BY created_at DESC LIMIT 5').bind(u.id).all();const rows=(sessions.results||[]).map(x=>x.id+' | '+x.game_type+' | stake '+x.stake+' | reward '+x.reward_applied+' | '+(x.result||'OPEN')).join('\\n');await send(env,chat,'🔧 *WALLET DEBUG*\\n━━━━━━━━━━━━━━\\n\\nUser ID: '+u.id+'\\nTelegram ID: '+id+'\\nWallet row: '+Number(w?.user_id??0)+'\\nBalance: '+Number(w?.balance??0)+'\\n\\n🎲 Game results\\nGames: '+Number(g?.count??0)+'\\nStaked: '+Number(g?.staked??0)+'\\nPayouts: '+Number(g?.payout??0)+'\\n\\n📊 Stats\\nGames: '+Number(s?.games_played??0)+'\\nWins: '+Number(s?.games_won??0)+'\\nLosses: '+Number(s?.games_lost??0)+'\\nStaked: '+Number(s?.total_staked??0)+'\\nWon: '+Number(s?.total_won??0)+'\\nLost: '+Number(s?.total_lost??0)+'\\n\\n🧩 Latest sessions\\n'+(rows||'None')+'\\n\\nV: owner-001-wallet-debug-primary-20260925-r3');return new Response('OK');}if(update.message?.text?.trim().split(/\\s+/)[0].toLowerCase()==='/menu'){await photo(env,update.message.chat.id,'🔥 *THE DAMPER_BOT V2*',mainMenu());return new Response('OK');}if(update.message){const handled=await handleSafeMessage(env,update.message);if(handled!==false)return new Response('OK');}if(update.callback_query){const handled=await safeCallback(env,update.callback_query);if(handled!==false)return new Response('OK');}}catch(e){console.error('safe wrapper error',e);}}return base.fetch(request,env,ctx);}};