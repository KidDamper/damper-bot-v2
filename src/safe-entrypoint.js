...
Nice reaction.`,backGames());}
  ...
  if(d==='economy_balance'){const balance=await getBalance(env,u.id);const xpRow=await env.DB.prepare('SELECT damper_xp,level FROM xp WHERE user_id=?').bind(u.id).first();const debug='\n\n🔧 *SYNC*\nUSER ID: USER ID: '+String(u.id)+' • WALLET: authoritative • V: wallet-system-20260922';return edit(env,chat,mid,'💰 *BALANCE*\n━━━━━━━━━━━━━━\n\n🪙 *Coins*\n'+balance+'\n\n⭐ *Level*\n'+(xpRow?.level||1)+'\n\n✨ *XP*\n'+(xpRow?.damper_xp||0)+debug,backMain());}
