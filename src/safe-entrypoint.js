...
async function user(env,id){const u=await env.DB.prepare('SELECT u.*,x.damper_xp,x.level,x.rpg_xp,x.rpg_level FROM users u LEFT JOIN xp x ON x.user_id=u.id WHERE u.telegram_id=?').bind(String(id)).first();if(!u)return u;const walletSession=env.DB.withSession('first-primary');const w=await walletSession.prepare('SELECT balance FROM wallets WHERE user_id=?').bind(u.id).first();return {...u,balance:Number(w?.balance??0)};}
...
async function handleSafeMessage(env,msg){if(!msg?.text||msg.text.startsWith('/'))return false;if(!(await allowed(env,msg.from.id)))return false;const u=await user(env,msg.from.id);if(!u)return false;const s=await activeSession(env,u.id,['SAFE_BRAIN','SOCIAL_TRUTH','SOCIAL_DARE','SOCIAL_RANDOM','SOCIAL_HOTSEAT','SOCIAL_IMPOSTOR']);if(!s)return false;const state=JSON.parse(s.state||'{}');
  if(s.game_type==='SAFE_BRAIN'){const correct=checkBrainAnswer(state.question,msg.text);const reward=brainReward(state.question,correct);if(reward.coins)await env.DB.prepare('UPDATE wallets SET balance=balance+? WHERE user_id=?').bind(reward.coins,u.id).run();await finishSafeSession(env,s.id,correct?'WIN':'LOSS');const next=nextQuestion(state.category);if(!next)return send(env,msg.chat.id,`${correct?'✅ Correct!':'❌ Not quite.'}${state.question.explanation?`
...
async function safeCallback(env,q){
  ...
  if(d.startsWith('safe_answer:')){const [,sid,raw]=d.split(':');const s=await env.DB.prepare('SELECT * FROM game_sessions WHERE id=? AND player_id=? AND result IS NULL').bind(sid,u.id).first();if(!s)return send(env,chat,'⌛ This question has ended.');const state=JSON.parse(s.state||'{}');const answer=(state.question.answers||[])[Number(raw)];if(answer===undefined)return send(env,chat,'⚠️ That answer button is no longer valid.',brainMenu());const correct=checkBrainAnswer(state.question,answer);const reward=brainReward(state.question,correct);if(reward.coins)await env.DB.prepare('UPDATE wallets SET balance=balance+? WHERE user_id=?').bind(reward.coins,u.id).run();await finishSafeSession(env,sid,correct?'WIN':'LOSS');const next=nextQuestion(state.category);if(!next)return edit(env,chat,mid,`${correct?'✅ Correct!':'❌ Not quite.'}
...
Nice reaction.`,backGames());}
  ...
  if(d==='economy_balance'){const walletSession=env.DB.withSession('first-primary');const check=await walletSession.prepare('SELECT balance FROM wallets WHERE user_id=?').bind(u.id).run();const xpRow=await env.DB.prepare('SELECT damper_xp,level FROM xp WHERE user_id=?').bind(u.id).first();const balance=Number(check.results?.[0]?.balance??0);const debug='\n\n🔧 *SYNC*\nID: '+u.id+' • DB: '+(check.meta?.served_by_primary??'unknown')+' • REGION: '+(check.meta?.served_by_region??'unknown')+' • V: wallet-authority-20260921';return edit(env,chat,mid,'💰 *BALANCE*\n━━━━━━━━━━━━━━\n\n🪙 *Coins*\n'+balance+'\n\n⭐ *Level*\n'+(xpRow?.level||1)+'\n\n✨ *XP*\n'+(xpRow?.damper_xp||0)+debug,backMain());}
