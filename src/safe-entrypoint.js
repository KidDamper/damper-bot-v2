...
const APP_VERSION='owner-001-wallet-debug-identity-20260925-r2';
...
export default {async fetch(request,env,ctx){if(request.method==='POST'&&new URL(request.url).pathname==='/telegram/webhook'){const clone=request.clone();try{const update=await clone.json();await ensureOwner(env,update.message);if(update.message?.text?.trim().split(/\s+/)[0].toLowerCase()==='/walletdebug'){const chat=update.message.chat.id,id=String(update.message.from.id);if(id!==OWNER_TELEGRAM_ID&&!(env.OWNER_TELEGRAM_ID&&String(env.OWNER_TELEGRAM_ID)===id)){await send(env,chat,'⛔ Creator access only.');return new Response('OK');}const u=await env.DB.prepare('SELECT id,telegram_id,username,damper_id,role FROM users WHERE telegram_id=?').bind(id).first();if(!u){await send(env,chat,'🔧 WALLET DEBUG\\n\\nNo users row found for Telegram ID: '+id);return new Response('OK');}if(String(u.role||'').toUpperCase()!=='OWNER'&&id!==OWNER_TELEGRAM_ID&&!(env.OWNER_TELEGRAM_ID&&String(env.OWNER_TELEGRAM_ID)===id)){await send(env,chat,'⛔ Creator access only.');return new Response('OK');}const w=await env.DB.prepare('SELECT user_id,balance FROM wallets WHERE user_id=?').bind(u.id).first();const g=await env.DB.prepare('SELECT COUNT(*) count,COALESCE(SUM(stake),0) staked,COALESCE(SUM(payout),0) payout FROM game_results WHERE user_id=?').bind(u.id).first();const s=await env.DB.prepare('SELECT games_played,games_won,games_lost,total_staked,total_won,total_lost FROM stats WHERE user_id=?').bind(u.id).first();const sessions=await env.DB.prepare('SELECT id,game_type,stake,result,reward_applied,created_at FROM game_sessions WHERE player_id=? ORDER BY created_at DESC LIMIT 5').bind(u.id).all();const rows=(sessions.results||[]).map(x=>x.id+' | '+x.game_type+' | stake '+x.stake+' | reward '+x.reward_applied+' | '+(x.result||'OPEN')).join('\\n');await send(env,chat,'🔧 *WALLET DEBUG*\\n━━━━━━━━━━━━━━\\n\\nUser ID: '+u.id+'\\nTelegram ID: '+id+'\\nWallet row: '+Number(w?.user_id??0)+'\\nBalance: '+Number(w?.balance??0)+'\\n\\n🎲 Game results\\nGames: '+Number(g?.count??0)+'\\nStaked: '+Number(g?.staked??0)+'\\nPayouts: '+Number(g?.payout??0)+'\\n\\n📊 Stats\\nGames: '+Number(s?.games_played??0)+'\\nWins: '+Number(s?.games_won??0)+'\\nLosses: '+Number(s?.games_lost??0)+'\\nStaked: '+Number(s?.total_staked??0)+'\\nWon: '+Number(s?.total_won??0)+'\\nLost: '+Number(s?.total_lost??0)+'\\n\\n🧩 Latest sessions\\n'+(rows||'None')+'\\n\\nV: owner-001-wallet-debug-identity-20260925-r2');return new Response('OK');}if(update.message?.text?.trim().split(/\\s+/)[0].toLowerCase()==='/menu'){await photo(env,update.message.chat.id,'🔥 *THE DAMPER_BOT V2*',mainMenu());return new Response('OK');}if(update.message){const handled=await handleSafeMessage(env,update.message);if(handled!==false)return new Response('OK');}if(update.callback_query){const handled=await safeCallback(env,update.callback_query);if(handled!==false)return new Response('OK');}}catch(e){console.error('safe wrapper error',e);}}return base.fetch(request,env,ctx);}};