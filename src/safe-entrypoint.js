...
Nice reaction.`,backGames());}
  ...
  if(d==='economy_balance'){const check=await env.DB.prepare('SELECT u.id AS user_id,u.telegram_id,w.user_id AS wallet_user_id,w.balance FROM users u LEFT JOIN wallets w ON w.user_id=u.id WHERE u.telegram_id=?').bind(String(q.from.id)).run();const row=check.results?.[0]||{};const xpRow=await env.DB.prepare('SELECT damper_xp,level FROM xp WHERE user_id=?').bind(u.id).first();const balance=Number(row.balance??0);const debug='\n\n🔧 *SYNC*\nUSER ID: '+String(row.user_id??'missing')+' • WALLET ID: '+String(row.wallet_user_id??'missing')+' • DB: '+(check.meta?.served_by_primary??'unknown')+' • REGION: '+(check.meta?.served_by_region??'unknown')+' • V: wallet-diag-20260921';return edit(env,chat,mid,'💰 *BALANCE*\n━━━━━━━━━━━━━━\n\n🪙 *Coins*\n'+balance+'\n\n⭐ *Level*\n'+(xpRow?.level||1)+'\n\n✨ *XP*\n'+(xpRow?.damper_xp||0)+debug,backMain());}
